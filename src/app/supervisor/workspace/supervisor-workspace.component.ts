...
export class SupervisorWorkspaceComponent implements OnInit {
  ...
  evaluationDraft = {
    ...
    notesAttribuees: [] as SupervisorEvaluationNote[]
  };
  ...
  openMeetingModal(meeting?: SupervisorMeeting): void {
    ...
    this.meetingDraft = meeting
      ? {
          ...
          date: meeting.date,
          heure: meeting.heure,
          typeReunion: meeting.typeReunion || 'HEBDOMADAIRE',
  ...
  saveReport(): void {
    ...
    this.isSaving = true;
    this.supervisorService.saveObservation(this.reportModal.id, this.reportText).pipe(timeout(15000)).subscribe({
      next: () => {
        this.isSaving = false;
  ...
  updateEvaluationNoteScore(note: SupervisorEvaluationNote): void {
