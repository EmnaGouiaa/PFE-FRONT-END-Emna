    ...
    <div class="page">
      <header class="page-header">
        <div>
          ...
          <p>Le responsable des stages valide les demandes. Des que la double validation est atteinte, l'entreprise est creee automatiquement.</p>
        </div>
      ...
      <div class="support-banner">
        Regle metier: l'entreprise est creee automatiquement en base des que l'administrateur et le responsable des stages approuvent la demande.
      </div>
...
export class FacultyCompanyRequestsPageComponent implements OnInit {
  ...
  approve(request: DemandeStage, event: Event): void {
    ...
    if (!this.canAct(request) || !confirm(`Approuver la demande #${request.id} cote responsable des stages ?`)) return;
    ...
    this.demandeStageService.validerResponsableStages(request.id).subscribe({
      next: (updatedRequest) => {
        this.successMessage = this.isFullyValidated(updatedRequest)
          ...
          : `Demande #${request.id} approuvee cote responsable des stages.`;
