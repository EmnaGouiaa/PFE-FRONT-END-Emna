...
export class StudentPortalService {
  ...
  updateMeetingReport(meetingId: number, compteRendu: string): Observable<StudentMeeting> {
    return this.http
      .put<any>(`${this.meetingsUrl}/${meetingId}/compte-rendu`, compteRendu.trim())
      .pipe(
        map((item) => this.normalizeMeeting(item, 'HEBDOMADAIRE')),
        catchError((error) => this.handleError(error))
      );
  }
